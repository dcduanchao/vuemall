<template>
  <div class="detail-shop-info" v-if="Object.keys(shop).length!==0">
    <div class="flex shop-name-info">
      <img :src="shop.shopLogo" class="shop-img">
      <div>{{shop.name}}</div>
    </div>
    ...
    <div class="flex shop-other-info">
      ...
      <div class="shop-other-item shop-other-right">
        <div v-for="item in shop.score" :key="item.name">
          {{item.name}}&ensp;
          <span class="score" :class="{active:item.better}">{{item.score}}</span>
          <span class="score-bg" :class="{active:item.better}">{{item.better? '高' : '低'}}</span>
        </div>
      </div>
    </div>
